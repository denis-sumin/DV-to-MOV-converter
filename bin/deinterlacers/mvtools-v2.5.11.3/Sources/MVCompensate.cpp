...
MVCompensate::MVCompensate(PClip _child, PClip _super, PClip vectors, bool sc, double _recursionPercent, int _thres, bool _fields,
...
super(_super)
{
   ...
   if (isse)
   {
      ...
      if (nBlkSizeY==16) {
         ...
         OVERSLUMA = Overlaps32x32_mmx;
		 if (yRatioUV==2) {
	         BLITCHROMA = Copy16x16_mmx;
	         OVERSCHROMA = Overlaps16x16_mmx;
		 } else {
	         BLITCHROMA = Copy16x32_mmx;
	         OVERSCHROMA = Overlaps16x32_mmx;
		 }
      } break;
      case 16:
      if (nBlkSizeY==16) {
   ...
   else // pure C, no isse opimization ("mmx" version could be used, but it's more like a debugging version)
   {
      ...
      if (nBlkSizeY==16) {
         ...
         BLITLUMA = Copy_C<32,32>;         OVERSLUMA = Overlaps_C<32,32>;
		 if (yRatioUV==2) {
	         BLITCHROMA = Copy_C<16,16>;	         OVERSCHROMA = Overlaps_C<16,16>;
		 } else {
	         BLITCHROMA = Copy_C<16,32>;	         OVERSCHROMA = Overlaps_C<16,32>;
		 }
      } break;
      case 16:
      if (nBlkSizeY==16) {
