...
//Chainik: make one really big array for ALL Y-vectors
	int sz=((nHeightP*VPitchY + 255) & (~255)) + 16;
	framesY = new BYTE [sz*2];

//and init pointers for convenience
	VXFullY=framesY;
	VYFullY=framesY+sz;

//Chainik: the same one for ALL UV-vectors
	sz=((nHeightPUV*VPitchUV + 255) & (~255)) + 16;
	framesUV = new BYTE [sz*2];

	VXFullUV=framesUV;
	VYFullUV=framesUV+sz;
//Chainik: everything below in MVFlow::MVFlow is unchanged
...
void MVFlow::Create_LUTYOffset(int time256, int *LUTYOffset, int ref_pitch)
{
	for (int v=0; v<256; v++)
		LUTYOffset[v] = ((v-128)*time256*ref_pitch)/256;
}
...
*/
				...
				int vy_ref_pitch = LUTYOffset[VYFull[w]];

				pdst[w] = pref[vy_ref_pitch + vx + w];
			}
			pref += ref_pitch;
			pdst += dst_pitch;
			VXFull += VXPitch;
...
*/
				...
				pdst[w] = pref[vy_ref_pitch + vx + (w<<1)];
				vx = LUTV[VXFull[w+1]];
				vy_ref_pitch = LUTYOffset[VYFull[w+1]];

				pdst[w+1] = pref[vy_ref_pitch + vx + (w<<1)+2];
			}
			pref += (ref_pitch)<<1;
			pdst += dst_pitch;
			VXFull += VXPitch;
