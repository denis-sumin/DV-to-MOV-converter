...
/* working fields */
   ...
   int nSrcPitch[3];
   const Uint8* pSrc[3]; // the alignment of this array is important for speed for some reason (cacheline?)
   int nRefPitch[3];
...
//   int nLambdaLen; //  penalty factor (lambda) for vector length
   ...
   int verybigSAD;
	...
	Uint8* pSrc_temp_base;	//stores base memory pointer to non _base pointer
	BYTE * dctSrc_base;		//stores base memory pointer to non _base pointer
	BYTE * dctRef_base;		//stores base memory pointer to non _base pointer
#endif
